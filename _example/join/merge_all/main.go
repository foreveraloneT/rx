...
func example1() {
	...
	source, _ := rx.Observable(func(observer rx.Observer[<-chan int]) {
		defer observer.Done()
		...
		source2, _ := rx.Map(rx.Interval(2000*time.Millisecond), func(v int, _ int) (int, error) {
			return v * 10, nil
		})
		observer.Next(source2) //  source 2 will emit at 3000, 5000, 7000, ...

		<-time.After(1000 * time.Millisecond)
		source3, _ := rx.Map(rx.Interval(1000*time.Millisecond), func(v int, _ int) (int, error) {
			return v * 100, nil
		})
	...
	/*
		Table to show relation between time and value emitting from each sources
	...
	*/
	...
	out := rx.Take(rx.MergeAll(source), 30)
