...
func example1() {
	...
	source, _ := rx.Observable(func(observer rx.Observer[<-chan int]) {
		...
		source2, _ := rx.Map(rx.Interval(1000*time.Millisecond), func(v int, _ int) (int, error) {
			return v * 10, nil
		})
		observer.Next(source2)

		<-time.After(3000 * time.Millisecond)
		source3, _ := rx.Map(rx.Interval(2000*time.Millisecond), func(v int, _ int) (int, error) {
			return v * 100, nil
		})
